...
def process_excel_file(filename):
    columns = ["argumentAttacking", "argumentAttacked", "relationship"]
...
def process_csv_file(filename):
    ...
    df_filtered = df.loc[df["relationship"].isin(['a', 's', 'n'])]
    return df_filtered
...
def construct_af(df):
    ...
    for index, relationship in df.iterrows():
...
def save_af_format(filename, args, af):
    ...
    args_filename = f"out/construct_AF_sp/{basename_without_ext}_args.pkl"
    graph_filename = f"out/construct_AF_sp/{basename_without_ext}_graph.af"
...
def save_net_format(filename, args, af):
    basename_without_ext = os.path.splitext(os.path.basename(filename))[0]
    args_filename = f"out/construct_AF_sp/{basename_without_ext}_args.pkl"
    graph_filename = f"out/construct_AF_sp/{basename_without_ext}_graph.net"
    # Save arguments abstraction dictionary
    with open(args_filename, 'wb') as f:
        pickle.dump(args, f)
    # Save graph in net format
