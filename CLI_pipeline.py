...
@app.command()
def run_pipeline(filepath:str, model:str="simple"):
    import Construct_pairs.construct_pairs as construct_pairs
    import Predict_relationships.predict_relationships as predict_relationships
    from InquirerPy import prompt
    import Construct_AF.construct_AF as construct_AF

    rprint(f"[green bold]Starting the pipeline with input data {filepath}.[green bold]")
    pairs_filename = construct_pairs.construct_pairs_subprocess(filepath)
    rprint(f"[green bold]Pairs of arguments successfully saved at {pairs_filename}.[green bold]")
    rprint(f"[grey]Predicting relationships...[grey]")
    relationships_filename = predict_relationships.run_predict_relationships_subprocess(pairs_filename, model)
...
def calculate_extensions(
    ...
    rprint("[yellow]=============================================[yello]")
    solver_path = "./Solvers/crustabri/crustabri/target/release/crustabri"
    
    if problem_code == "SE":
        solver_command = f"{solver_path} solve -p {subtrack} -f {graph_filepath}"
    ...
    if problem_code == "DS" or problem_code == "DC":
        ...
        solver_command = f"{solver_path} solve -p {subtrack} -f {graph_filepath} -a {arg_code}"
        # Run solver with params
        output = subprocess.check_output(
            solver_command,
            shell=True,
